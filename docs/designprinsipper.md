...
| **Medlemmer** | `src/routes/medlemmer/index.tsx` | Admin (seksjonsleder i redusert form) | Invitere et medlem og sette rolle + stemme — og for alle andre: finne kontaktinfoen til den man trenger tak i | Lesing: `requireMe()`; skriving: `members.manage` / `members.manage.section`. Kontaktinfo (#14) er synlig for alle innloggede medlemmer, men fjernes server-side for deaktiverte (`redactContact` i `src/lib/member-profile.ts`). Egne profilfelt redigeres på `/min-profil`, andres med `members.manage` |
