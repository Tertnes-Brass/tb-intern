...
- **Tabell-rebuild i D1:** drizzle-kit genererer `PRAGMA foreign_keys=OFF` + `CREATE __new_x` / `DROP TABLE x` / `RENAME` når en kolonne endres. D1 kjører migrasjonen i en transaksjon, der PRAGMA-en er en **no-op** — `DROP TABLE` cascader altså til barnetabellene og sletter data i stillhet. `migrations/0010_wall.sql` sikrer derfor `notification_log` i en midlertidig tabell og legger radene tilbake etterpå. Sjekk alltid hvem som peker på tabellen før du lar en generert rebuild passere.
