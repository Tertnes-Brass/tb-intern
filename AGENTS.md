...
(`src/routes/styre/route.tsx` med områdemenyen Oppgaver · Møter · Dokumenter),
gated på `board.manage`. `src/routes/index.tsx` (`/`) er
...
- Styre: `/styre` er styrets eget område (oppgaver, møter, kommentarer,
  dokumenter), gated på `board.manage` — også for lesing.
  `src/server/board.ts` har serverfunksjonene, `src/lib/board.ts` den rene
  logikken (sortering, «forfalt», gruppering) med tester. Dokumenter ligger i
  ...
  `api/files/$fileId`. `src/server/board-files.ts` er R2-laget (levende eksport
  som rører `cloudflare:workers`; importeres aldri fra en rutekomponent).
