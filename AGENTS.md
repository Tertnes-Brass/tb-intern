...
- **Utgivelsesnotat:** `docs/utgivelser/neste.md` beskriver det som ligger på staging og venter på verifisering. Den som integrerer en runde skriver det (for medlemmene som skal teste, ikke for utviklere); deploy-workflowene poster det automatisk til Discord-kanalen #dev — ved staging med «må testes før main», ved prod som «ute i prod». Etter prod: flytt til `docs/utgivelser/ÅÅÅÅ-MM-DD.md`. Se `docs/utgivelser/README.md`.
...
- **Målretting og lest/sett på veggen (#28):** to additive tabeller (migrasjon `0017_maalretting-og-sett.sql`, kun CREATE TABLE + CREATE INDEX). `post_targets` (PK `(post_id, kind, ref_id)`, FK CASCADE på innlegget) er en **innsnevring oppå `audience`**, aldri en utvidelse: `audience` avgjør fortsatt hele korpset vs. bare styret, målrettingen svarer på «hvem av dem», og en beskjed UTEN rader her oppfører seg NØYAKTIG som før — det er derfor `audience` ikke ble bygget om. `kind` er `'section'` (en verdi fra `parts.section`, altså stemmegruppa «slagverk», ikke enkeltstemmen «Slagverk 2») eller `'project'` (`projects.id`). `ref_id` har med vilje INGEN fremmednøkkel: kolonnen betyr to ting avhengig av `kind`, og for seksjoner finnes det ikke en tabell å peke på. Forsvinner prosjektet, blir raden foreldreløs og beskjeden treffer ingen (fail-closed), men står lesbar for forfatteren og `posts.publish` — foreldreløse rader ryddes aldri automatisk, som ellers i basen.
  - **Regelen bor ett sted.** `inPostAudience` i `src/lib/posts.ts` er den ENE definisjonen av «er dette medlemmet i målgruppen», og både `canReadPost` (lesing), `recipientsFor` (e-post) og `postAudienceMembers` (nevneren i «Sett av N av M») går gjennom den. `PostReader` er `{ canPublish, sectionIds, projectIds }`; `canReadPost` godtar fortsatt en bar `boolean` for gamle kallsteder, og den er da fail-closed (en leser uten stemmer treffes ikke av noen målretting). Merk skillet «kan lese» ≠ «er mottaker»: en `posts.publish`-holder kan lese en beskjed til slagverksgruppa (moderasjon), men telles ikke som mottaker av den og får ikke e-posten.
  - **Hvem kan målrette:** samme rettighet som styre-målgruppen, `posts.publish`. `sanitizePostTargets` tømmer lista for alle andre, akkurat som `sanitizePostInput` tvinger `audience` til `all`, og `updatePost` rører ikke målrettingen uten rettigheten. `assertValidTargets` avviser en stemmegruppe eller et prosjekt som ikke finnes — et rått kall kan ikke dikte opp en gruppe.
  - **Prosjektmedlemskap FINNES IKKE i datamodellen** og er avledet i `src/server/post-audience.ts` (`projectsByUser`): den som har svart `attending`/`unsure` på minst én øving koblet til prosjektet (`event_projects` → `event_attendance`), pluss den som står som ansvarlig i prosjektets tidsplan (`project_times.responsible_user_id`). Den kjente kanten er at **den som ikke har svart, ikke er med** — derfor viser målrettingsvelgeren ANTALL medlemmer per valg (`listPostTargetOptions`), så valget tas med tallet synlig. `sectionIds` kommer fra `user_parts` ⨝ `parts`, ALDRI fra `member_instruments`: bistemmer er noe medlemmet krysser av selv, og ville latt hvem som helst skrive seg inn i slagverkets beskjeder.
  - **`post-audience.ts` er en egen modul** av samme grunn som `post-images.ts`: den har levende eksporter som også kalles fra bilderuta `/api/post-images/$imageId`, og lå de i `posts.ts` ville `cloudflare:workers` blitt dratt inn i klientbygget. Synlighetsregelen gjentas nå tre steder, alle server-side: feeden filtrerer i JS etter henting (`listPosts`), hub-en filtrerer i SQL (`targetsAllowReader`, fordi den henter et fast lite antall rader og et etterfilter ville gjort «tre siste» til null), og bildegaten sjekker per bilde.
  - **`post_seen`** (PK `(post_id, user_id)`, begge FK-er CASCADE) skrives idempotent i `getPost` når et medlem åpner detaljsiden — `seen_at` er FØRSTE gang, ikke siste, og `onConflictDoNothing` gjør et nytt besøk til en no-op. Utkast og forfatterens eget besøk registreres ikke. **Ingen e-post og ingen push henger på tabellen.** Tallet «Sett av N av M» vises for forfatteren og `posts.publish` (`canSeeSeenStatus`), mens NAVNELISTA er strengere: kun for beskjeder merket «Viktig» (`canSeeSeenNames`) — issue-kravet er at stab ser hvem som har sett en viktig kunngjøring, ikke hvem som leste en hilsen. Nevneren er `postAudienceMembers`: målgruppen uten forfatteren, så en beskjed til slagverk viser «2 av 3» og ikke «2 av 34». Serveren sender `seen: null` / `names: null` til dem som ikke skal se det — UI-et filtrerer ingenting.
...
- **Sosiale arrangement (#31):** pub etter øving, julebord, fjelltur, dugnad. Egne LOKALE arrangement i `social_events`/`social_signups` (migrasjon `0017_sosiale-arrangement.sql`, kun CREATE TABLE + CREATE INDEX) — de kommer IKKE fra Google-feeden, har egen id og egen detaljrute `/kalender/sosialt/$socialId`, og er bevisst holdt UTENFOR `occurrenceKey`-verdenen: den nøkkelen forutsetter en hendelse i feeden å utlede seg fra. De vises likevel i kalenderlista sammen med feed-hendelsene, merket «Sosialt» — sammenslåingen er `mergeCalendarRows` i `src/lib/social.ts`, delt av `/kalender` og hub-ens «Kommende» slik at de to listene ikke kan sortere ulikt. Fire ting er ikke til forhandling:
  - **Alle aktive medlemmer kan opprette.** `createSocialEvent` krever bare `requireMe()`, ingen rettighet — samme valg som veggen (#28) tok, av samme grunn: skal plattformen erstatte Facebook-gruppen, må terskelen for å foreslå en fjelltur være lav. Redigering og avlysning er arrangørens eget pluss moderasjon (`canEditSocialEvent` = eier ∨ `members.manage` ∨ `*`). Å svare er alltid ens eget svar: `setMySignup` har ingen `userId`-parameter, så ingen kan melde på noen andre.
  - **Ventelista fordeles ved LESING, ikke ved skriving.** `splitByCapacity` sorterer «kommer»-radene på `attending_since` og deler dem i plass/venteliste. Derfor trengs ingen `insert … where (select count(*)) < max`, som er greit siden D1 ikke har interaktive transaksjoner — og melder én av de påmeldte avbud, rykker nummer én opp i samme øyeblikk, uten et opprydningssteg som kan glemmes. `attending_since` settes KUN når svaret går fra noe annet til «kommer»: en som retter kommentaren sin beholder plassen, en som melder avbud og ombestemmer seg stiller bakerst.
  - **Fristen stenger påmelding, ikke avbud.** `signupState` er én funksjon som både serveren og skjermen bruker (`allowed`/`canClear`/`message`), så knappene og valideringen ikke kan komme i utakt. Etter fristen er `attending` stengt mens `not_attending` står åpen — å låse svaret helt ville betydd at den som blir syk enten møter opp eller lar plassen stå tom i stedet for å gå til ventelista. Avlyst og «har vært» låser alt.
  - **Ingen sletting.** Avlysning er myk (`cancelled_at`, kan angres) og raden blir stående merket «Avlyst» med påmeldingene i behold — den som har svart «kommer» skal se HVA som ble avlyst, ikke oppdage at det forsvant. Deltakerlista er åpen for alle medlemmer, i motsetning til oppmøtelista (#82/#24): der er spørsmålet hvem som ikke kom på øvelsen, her er det hvem du møter på julebordet. Ingen e-postvarsling i denne omgangen; modellen er skrevet så det blir additivt (mottakerlista er `social_signups`, idempotensen får en egen loggtabell etter mønster fra `notification_log`).
- **Veggklokke → UTC bor i `src/lib/wallclock.ts`** (`wallClockToUtc`, `utcToWallClock`, `floatingToUtc`, `zoneOffsetMs`), trukket ut av `ical.ts` da #31 trengte den samme regningen. Skjemaet sender dato + klokkeslett som NORSK VEGGKLOKKE og serveren regner om; `new Date('2026-12-12T19:00')` i nettleseren ville blitt tolket i klientens tidssone og lagt julebordet feil for den som melder det inn fra utlandet. To kopier av omleggingshåndteringen ville før eller siden gitt to svar på hva 02:30 den siste søndagen i mars er.
...
- **Solister, slagverksinstrumenter og øvingsstatus (#50 + #34 + #30):** tre ting som henger på repertoaret, med hvert sitt eierskapsnivå — og det nivået er hele poenget i hver av dem. Migrasjon `0017_solister-slagverk-ovingsstatus.sql` (kun CREATE TABLE + CREATE INDEX). Ingen nye rettigheter: alle tre gjenbruker `projects.manage`, `works.manage` og `calendar.manage`.
  - **Solister (#50) hører til KOBLINGEN prosjekt↔verk.** `project_work_soloists` har en SAMMENSATT fremmednøkkel mot `project_works(project_id, work_id)` med CASCADE: «Napoli» har ikke en solist, «Napoli på julekonserten» har det, og neste gang stykket spilles starter man blankt. En solist er ENTEN et medlem (`user_id`, navnet slås opp ferskt i `user` — aldri lagret som kopi) ELLER et fritekstnavn (`external_name`) for vikaren, gjesten eller gruppa; `parseSoloistInput` i `src/lib/soloists.ts` lar medlemmet vinne når begge er fylt ut, slik at én rad aldri kan bære to navn. **Solistgruppe ble bevisst IKKE en egen type** (saken spurte): «Trombonegruppa» er et fritekstnavn, og `role` bærer nyansen — en gruppe i et konsertprogram er sjelden nøyaktig stemmegruppa i besetningen. `user_id` er SET NULL som i tidsplanen: slutter solisten, står programmet.
  - **Slagverksinstrumenter (#34) hører til VERKET.** `work_percussion` (instrument, fritekstnotat, valgfri slagverksstemme) følger stykket og gjenbrukes hver gang det settes opp — det er hele saken: slippe å lese instrumentene ut av PDF-ene på nytt. Ikke å forveksle med `project_works.percussion_setup`, som er FORDELINGEN for én konsert («Timpani – Silje»); to kolonner med samme navn ville gitt to svar. `part_id` er SET NULL og valideres server-side mot seksjonen `perc` (`isPercussionPart`) — en instrumentlinje merket «2. kornett» skal et rått kall ikke kunne skrive. Lesingen er **ikke** gated på slagverksstemme slik `showPercussionFor` gater fordelingen: en riggeliste skal kunne leses av alle som er med og bærer. Prosjektsiden og den utskriftsvennlige `/slagverk` viser det samlede behovet på tvers av repertoaret (`aggregatePercussionNeeds` slår sammen på instrumentnavn i små bokstaver). PDF-/OCR-forslag er IKKE bygget; ingenting i modellen forutsetter hvordan raden ble til, så et forslagssteg kan skrive de samme radene senere.
  - **Øvingsstatus (#30) hører til MEDLEMMET.** `project_work_practice` (PK `(project_id, work_id, user_id)`) har bevisst INGEN `source`/`registered_by` slik `event_attendance` har: ingen kan sette en status for noen andre, `user_id` kommer alltid fra `requireMe()`, og det finnes ingen serverfunksjon som skriver for en tredjepart. Fraværsføring er en oppgave noen har fått; øvingsstatus er noe man sier om seg selv. Innsynet følger `practiceScope` i `src/lib/practice.ts` — bygget LIKT `attendanceScope` (all / sections / self), men med `projects.manage` ∨ `calendar.manage`, og gruppelederen ser sine egne stemmer via `me.leadsPartIds`. **Tonen er et krav, ikke pynt:** `PracticeCounts.total` teller dem som HAR sagt noe og aldri antall medlemmer, så «hvem mangler status» ikke kan regnes ut av det klienten får; å trykke på en status man har, fjerner den; og «trenger hjelp» heter «Vil øve med noen» og vises som et ønske noen kan svare på, aldri som en avviksliste. Testene i `practice.test.ts` låser dette.
  - **Synlighetsregelen for ett prosjekt er hentet ut** til `src/lib/project-access.ts` (`projectVisibility` + `isActiveProject`), fordi øvingsstatusen fikk en egen skrivevei: `getProject` og `setMyPracticeStatus` må svare likt på «ser du dette prosjektet?», og to steder som gjentar samme trapp av `if`-er gir før eller siden hvert sitt svar. Cascade-oppførselen til de tre tabellene er bevist mot ekte SQLite med `PRAGMA foreign_keys = ON` i `src/server/project-work-extras-migration.test.ts`.
- **Delte stemmer mellom medlemmer (#16):** et medlem kan la et annet medlem LESE notene til én av sine egne tildelte stemmer — «noe lavt i horn som skal prøves på baryton». Tabellen er `part_shares` (migrasjon `0017_stemmedeling-mellom-medlemmer.sql`, kun CREATE TABLE + CREATE INDEX; PK `(from_user_id, to_user_id, part_id)`, alle tre FK-er CASCADE), de rene reglene ligger i `src/lib/part-shares.ts` og serverfunksjonene i `src/server/part-shares.ts`. Tre ting er ikke til forhandling. **(1) Delingen er avledet, ikke lagret tilgang:** `currentUser()` join-er `part_shares` mot delerens `user_parts` og `member_profiles.is_active`, så en deling aldri kan gi mer enn deleren har akkurat nå, og den slutter å virke av seg selv når stemmen eller medlemskapet forsvinner — ingen rad å rydde. **(2) Håndhevelsen skjer ETT sted:** den delte stemmen kommer inn som `AccessCtx.sharedParts` og vurderes av `memberCanAccessPart` i `file-access.ts`, så fil-gaten, `assembleRepertoire` og `memberCanSeeFile` arver regelen gratis. Ingen parallell sjekk i UI-laget. **(3) Delingen gir aldri mer enn `part`-filer:** partitur følger `scores.view`, uplassert følger `archive.viewAll`, og `canViewAll` leses alltid av MOTTAKERENS kontekst. Man kan bare dele stemmer i sin egen `user_parts`, så en mottatt deling kan ikke deles videre og en seksjonsleder kan ikke dele stemmene hen bare *leder*. `updateMemberParts` sletter delinger av stemmer medlemmet mister, slik at ingen rad kan våkne til live igjen ved en senere tildeling. Ingen ny rettighet (gaten er eierskapet), ingen utløpsdato (begge parter kan fjerne den; taket `MAX_PART_SHARES` = 10 er en veiledende skranke, ikke en sikkerhetsgrense), ingen slagverksoppsett (det gjelder hele konserten og ville gitt mer enn stemme-lesing), ingen e-post — mottakeren ser den under «Delt med deg av Navn» på «Mine noter». **Vikarlenkene (`share_links`, `/v/$token`) er urørt** og forblir verktøyet for folk uten konto. Se `docs/tilgangsstyring.md` §5e.
