...
ett område i den. Beskjeder fra styret bor i `/beskjeder`
(`src/routes/beskjeder/{index,ny,$postId/index,$postId/rediger}.tsx`).
...
- Beskjeder (#28): `src/server/posts.ts` (feed, detalj, utkast, publisering, varsling) bak `requireMe()` for lesing og `posts.publish` for skriving. Utkast og `audience: 'board'` filtreres ALLTID server-side — `canReadPost` i `src/lib/posts.ts` er regelen, og den brukes både i `listPosts`, `getPost` og hub-en. Ren logikk (utdrag, avsnitt/auto-lenking med HTML-escaping, `recipientsFor`, kvitteringsteksten) bor i `src/lib/posts.ts` og er testet i `posts.test.ts`. Teksten er ren tekst med avsnitt, aldri markdown eller betrodd HTML.
...
- Roller: `SEED_ROLES` i `src/server/seed-data.ts` (`admin`, `archivist`, `conductor`, `board` «Styremedlem», `member`). Roller seedes bare i en tom database (`seedBaseConfig`), så en ny systemrolle krever også en migrasjon med `INSERT OR IGNORE` — se `migrations/0008_board-role.sql`. Samme gjelder nye rettigheter på eksisterende roller (`migrations/0009_posts.sql` gir `board` og `conductor` rettigheten `posts.publish`). NB: `INSERT OR IGNORE` dekker ikke fremmednøkler — skriv `INSERT ... SELECT id, '<rettighet>' FROM roles WHERE id IN (…)` så migrasjonen også går gjennom i en database uten roller.
